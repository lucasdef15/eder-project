...
const Stepper = ({ currentStep = 1, stepRefs, cardRefs }) => {
  ...
  return (
    <div className='w-full flex flex-col items-center px-4 overflow-hidden'>
      <div className='relative flex justify-between items-center w-full max-w-5xl mt-8'>
        <div
        ...
        {steps.map((step, index) => (
          <div
            key={step.number}
            className='z-10 flex flex-col items-center w-1/4 md:w-1/5'
          >
            <div
              ref={(el) => (stepRefs.current[index] = el)}
              className={`w-12 h-12 flex items-center justify-center rounded-full font-semibold text-lg shadow-md transition-colors ${
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 mt-8 w-full max-w-5xl text-center'>
        {steps.map((step, index) => (
          <div
            key={step.number}
            ...
            className='space-y-2'
          >
            <h3 className='text-lg sm:text-xl md:text-2xl font-semibold text-emerald-800'>
              {step.title}
            </h3>
